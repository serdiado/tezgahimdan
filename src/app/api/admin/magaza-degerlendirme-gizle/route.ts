...
// Admin moderasyon: magaza-seviyesi MagazaDegerlendirme.gizliMi bayragini yaz.
// api/admin/degerlendirme-gizle/route.ts ile AYNI desen, ayri tablo.
export async function POST(request: Request) {
  ...
  const degerlendirme = await prisma.magazaDegerlendirme.findUnique({
  ...
  await prisma.$transaction([
    prisma.magazaDegerlendirme.update({ where: { id: degerlendirmeId }, data: { gizliMi: gizle } }),
    prisma.durumGecmisi.create({
      data: {
        kullaniciId: session.user.id,
        varlikTuru: "MagazaDegerlendirme",
        varlikId: degerlendirmeId,
        olay: gizle ? "magaza_degerlendirme_gizlendi" : "magaza_degerlendirme_tekrar_gorunur",
      },
    }),
  ]);
