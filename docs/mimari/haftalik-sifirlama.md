...
3. **Vitrin "beklemede" görünümü** (`pasifUrunIdSeti` + `UrunKarti.tsx`/`UrunDetayModal.tsx`/mağaza sayfası/anasayfa): ilgili ürün, `Urun.durum`'a hiç dokunulmadan, alıcıya gösterilen vitrinde "Beklemede" rozetiyle işaretlenir ve "Rezerve Et" kapatılır — belirsizlik çözülene kadar o ürüne yeni rezervasyon eklenemez (kuyruk daha fazla karmaşıklaşmasın diye).
...
- Kuyruk (bu iptal grubuyla) boşalınca ürün `satildi` değilse → `sergide`. **Not:** başlangıçta-aktif-ve-işaretlenmemiş kayıt hâlâ varsa (satıcı henüz işaretlemediyse) ürün vitrinde ayrıca `pasifUrunIdSeti` katmanıyla "Beklemede" gösterilir — bu, `Urun.durum`'dan BAĞIMSIZ bir görünürlük katmanı.
