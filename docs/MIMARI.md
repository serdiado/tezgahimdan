...
Pazar **gün sonunda** (gece yarısı — 2026-07-09'dan önce kapanış ANINDAydı, kullanıcı kararıyla satıcıya işaretleme süresi tanımak için ertelendi) o pazarın bekleyen kuyruğunu **tamamen temizler** (rezervasyon motoruyla **aynı** `FOR UPDATE` kilidi, ama davranış ayrı — motor yükseltir, sıfırlama temizler). No-show cezası pazar **başlangıç** eşiğine göre (bu değişmedi): `aktifOlmaZamani < pazarBaslangicAni` olan (başlangıçta zaten aktif) + satılmamış → `gelmedi`; sonradan yükselen + yedekler → cezasız `iptal`; `satildi` dokunulmaz. Kapanıştan 1 saat sonra, işaretlenmemiş satıcılara ayrı bir hatırlatma bildirimi gider (`pazarHatirlatmalariGonder`). Harici cron + korumalı API (`/api/cron/pazar-sifirlama`), duruma-bakan (restart'ta kaçmaz), idempotent (`PazarSifirlama` + `PazarHatirlatma` tabloları). No-show olayları puan sistemine `rezervasyon_gelmedi:otomatik:%` formatında hazır. **Ertelenen karar:** satıcı gün sonunda da işaretlemezse ne olacağı henüz netleşmedi.
