...
Pazar **işlem-sonu anında** (2026-07-09'dan önce kapanış ANINDAydı, sonra "gün sonu/gece yarısı"na, sonra da tam admin-yapılandırılabilir bir alana evrildi) o pazarın bekleyen kuyruğunu **tamamen temizler** (rezervasyon motoruyla **aynı** `FOR UPDATE` kilidi, ama davranış ayrı — motor yükseltir, sıfırlama temizler). No-show cezası pazar **başlangıç** eşiğine göre (bu hiç değişmedi): `aktifOlmaZamani < pazarBaslangicAni` olan (başlangıçta zaten aktif) + satılmamış → `gelmedi`; sonradan yükselen + yedekler → cezasız `iptal`; `satildi` dokunulmaz. Hem "işlem sonu" (cezanın uygulandığı an) hem "hatırlatma" (satıcıya "unutma" bildiriminin gittiği an) admin panelinden **pazar bazında manuel ayarlanabilir** (`Pazar.islemSonGunu/Saati`, `hatirlatmaGunu/Saati` — boş bırakılırsa sırasıyla kapanış-gününün-gece-yarısı / kapanış+1saat varsayılır) — gece geç saatlere kadar açık pazarlarda sabit varsayımlar yanlış olacağı için. Harici cron + korumalı API (`/api/cron/pazar-sifirlama`), duruma-bakan (restart'ta kaçmaz), idempotent (`PazarSifirlama` + `PazarHatirlatma` tabloları). No-show olayları puan sistemine `rezervasyon_gelmedi:otomatik:%` formatında hazır. **Ertelenen karar:** satıcı işlem-sonunda da işaretlemezse ne olacağı henüz netleşmedi.
